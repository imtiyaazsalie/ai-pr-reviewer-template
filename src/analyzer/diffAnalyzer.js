...
try {
  const raw = fs.readFileSync('config/monorepo.yml', 'utf8');
  config = yaml.load(raw);
} catch (e) { /* no config */ }

const filtered = files.filter(f => {
  if (config.ignore && config.ignore.some(pattern => f.match(new RegExp(pattern.replace('*', '.*'))))) {
    return false;
  }
  if (config.workspaces && config.workspaces.length) {
    return config.workspaces.some(ws => f.startsWith(ws.replace('*', '')));
  }
  return !f.includes('lock') && !f.includes('dist') && !f.includes('node_modules') && !f.includes('.md');
});
